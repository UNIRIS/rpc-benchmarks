defmodule RanchAvro.Handler do
  use GenServer

  @operation_codes ["01"]

  def start_link(ref, socket, transport, _opts) do
    schema = File.read!("schema.avsc")
    pid = :proc_lib.spawn_link(__MODULE__, :init, [schema, ref, socket, transport])
    {:ok, pid}
  end

  def init(schema, ref, socket, transport) do
        ...
        case Map.get(req_buffer, socket_id) do
          ...
          socket_buf ->
            ...
            if remaining > 0 and byte_size(chunk) == remaining do
              ...
              req_data =  decoder.(socket_buf.data <> chunk)
              case handle_operation(socket_buf.id, req_data) do
                ...
                data ->
                  ...
                  res = encoder.(data) |> IO.iodata_to_binary
