defmodule RanchAvro.Client do
  ...
  def multi_calls(nb) do
    ...
    IO.inspect(1..nb
    |> Enum.map(fn _ -> Task.async fn -> RanchAvro.Client.do_call(tx, schema_json) end end)
  ...
  def delay_multi_calls(nb) do
    ...
    IO.inspect(1..nb
    |> Enum.map(fn _ ->
      t = Task.async fn -> RanchAvro.Client.do_call(tx, schema_json) end
      Process.sleep(100)
      t
    end)
    |> Enum.map(fn t -> Task.await(t, :infinity) end), limit: :infinity)
  ...
  def do_call(tx, schema_json) do

    start = System.os_time(:millisecond)

   {:ok, socket} = :gen_tcp.connect('localhost', 10000, [:binary, packet: 0, active: :once])
    encoder = :avro.make_simple_encoder(schema_json, [])
    tx_raw = encoder.(tx) |> IO.iodata_to_binary
    ...
    decoder = :avro.make_simple_decoder(schema_json, [])
    decoder.(res)
  ...
  defp create_transaction(pub, sig) do
    ...
    node_movements =
      node_movements ++
        for _i <- 1..50, do: %{fee: 0.5, node_public_key: pub, is_data_giver: true}
