defmodule RanchRpc.Client do
  ...
  def multi_calls(nb) do
    ...
    IO.inspect(1..nb
    |> Enum.map(fn _ -> Task.async fn -> RanchRpc.Client.do_call(tx) end end)
  ...
  def delay_multi_calls(nb) do
    ...
    IO.inspect(1..nb
    |> Enum.map(fn _ ->
      t = Task.async fn -> RanchRpc.Client.do_call(tx) end
      Process.sleep(100)
      t
    end)
    |> Enum.map(fn t -> Task.await(t, :infinity) end), limit: :infinity)
  ...
  def do_call(tx) do
   ...
   {:ok, socket} = :gen_tcp.connect('localhost', 6000, [:binary, packet: 0, active: :once])
    tx_raw = :erlang.term_to_binary(tx)
